...
paris_scraper = ParisScraper(output_dir="data")
...
if st.button("🚀 Start Scraping"):
    ...
    if not product_inputs:
        ...
        for query in product_inputs:
            ...
            results = paris_scraper.scrape_paris_products(query, max_products=max_products)
            final_data.extend(results)

        unique_products = {}
        for row in final_data:
            if row['product_id'] not in unique_products:
                unique_products[row['product_id']] = row

        final_data = list(unique_products.values())
        st.session_state["scraped_data"] = final_data  # store in session
        paris_scraper.save_to_csv(final_data, filename="product_reviews.csv")
